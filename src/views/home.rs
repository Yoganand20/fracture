...
pub fn Home() -> Element {
    ...
    rsx! {
        div { class: "flex flex-col h-full w-full overflow-hidden",
            ...
            // Container: Occupies all remaining available space, completely hidden scrollbars
            div { class: "flex-1 p-4 flex flex-col justify-between items-center min-h-0 overflow-hidden",

                // the shield and dynamic text right at the very top
                div { class: "w-full flex flex-col justify-start items-center space-y-5 pt-2",
                    ...
                    div { class: "py-1",
                        Icon {
                            data: lucide::Shield,
                            size: "120",
                            class: if is_active() { "text-orange-500 drop-shadow-[0_0_15px_rgba(249,115,22,0.4)] transition-all duration-300" } else if is_dark() { "text-zinc-800 transition-all duration-300" } else { "text-zinc-300 transition-all duration-300" },
                        }
                    }
                    ...
                    div { class: "text-center space-y-1",
                        h2 {
                            class: "text-xl font-extrabold tracking-tight ".to_owned()
                                + if is_dark() { "text-zinc-100" } else { "text-zinc-900" },
                            if is_active() {
                                "Tunnel is Active"
                            } else {
                                "No Active Tunnels"
                        ...
                        p {
                            class: "text-xs px-6 ".to_owned()
                                + if is_dark() { "text-zinc-400" } else { "text-zinc-500" },
                            if is_active() {
                                "DPI bypass is currently routing your traffic."
                            } else {
                                "Click start to enable DPI bypass security protections."
                            }
                        }
                    }
                }
            ...
            div {
                ...
                button {
                    class: if is_active() { "px-4 py-3 w-full bg-transparent border-2 border-orange-500/60 text-orange-500 font-bold rounded-xl hover:bg-orange-500/10 transition-all" } else { "px-4 py-3 w-full bg-orange-500 text-zinc-950 font-bold rounded-xl hover:bg-orange-600 shadow-[0_0_15px_rgba(249,115,22,0.25)] transition-all" },
